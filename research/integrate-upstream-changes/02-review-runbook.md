...
read `01-ledger-format.md` for the entry schema it produces.
...
mkdir -p research/integrate-upstream-changes/log
```
...
Intersect the touched paths with `path-map.yaml`:
...
- **any path is `unknown`** → resolve it: check whether the file exists here
  ...
  `path-map.yaml`. The map only stays useful if triage feeds it.
- **anything else** → §4.
...
3. **`replace`, `variant`, or `new`?** Apply the test from `00-problem-and-design.md` §3.5:
...
bd create "Port upstream <sha>: <subject>" \
   ...
   -d "Upstream <sha>. Ledger: research/integrate-upstream-changes/log/<sha>.md"
```
...
Append the batch record to `state.yaml` (see `01-ledger-format.md` §2): totals, merges
...
```bash
git add research/integrate-upstream-changes
...
code. Implementation happens later, from `bd`, on its own branch. When the work lands, add
`integrated: <our-sha>` to the ledger entry and close the bd issue.

Useful extraction commands for the implementer:

```bash
git show <sha> -- <path> > /tmp/upstream.patch    # inspect
git cherry-pick -n <sha>                          # verbatim attempt; expect conflicts
git checkout <sha> -- <path>                      # take a whole file at that revision
```

`git cherry-pick -n` is worth trying first even when the verdict is `adapt`: the conflict
set is itself a precise map of where our architecture diverges.

## 7. Standing queries

```bash
# what is still untriaged
rg -l 'verdict: pending' research/integrate-upstream-changes/log/

# accepted but not yet implemented
rg -l 'verdict: (adopt|adapt)' research/integrate-upstream-changes/log/ \
  | xargs rg -L 'integrated:'

# everything we rejected, with reasons
rg -l 'verdict: reject' research/integrate-upstream-changes/log/ | xargs rg -A3 '## Why not'

# seam gaps found so far
rg -A4 '## Seam feedback' research/integrate-upstream-changes/log/
...
top of `00-problem-and-design.md`, so a future reader knows the process ended deliberately
rather than lapsed.
