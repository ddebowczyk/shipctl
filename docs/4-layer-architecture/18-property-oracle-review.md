...
| `PROP-G-LAYMAN-MOVE-001` | The pure source-removal and target-append model can omit a Layman command-gate condition. | Apply the emitted action through the public workspace bridge and compare the canonical projected stacks. | Bridge and authority can share an incorrect semantic move rule. |
| `PROP-G-LAYMAN-SPLIT-001` | The pure edge-direction table can omit a Layman command-gate condition. | Route an ID-free split through the public bridge and validate authority-produced topology and identities in generated histories. | Bridge and authority can share an incorrect split rule. |
