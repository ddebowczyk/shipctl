...
//!
//! ```sh
//! cargo test -p shipctl-module-semantic-terminal-core --lib traces::tests::record -- --ignored
//! ```
//!
...
mod tests {
    ...
    fn recorded_bytes(trace: &Trace) -> Vec<u8> {
        fs::read(bytes_path(trace)).unwrap_or_else(|error| {
            panic!(
                "trace {} is missing its recording ({error}); record it with `cargo test -p shipctl-module-semantic-terminal-core --lib traces::tests::record -- --ignored`",
                trace.name
            )
        })
    }
