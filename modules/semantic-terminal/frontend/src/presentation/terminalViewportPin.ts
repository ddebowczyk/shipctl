 ...
 *
 * `terminalScrollPin.ts` classifies the gesture. This module decides what a
 * completed drain owes the viewport and applies those
...
export function resolveViewportDrainAction(input: {
  pinnedToBottom: boolean;
  pendingBottomOffset: number | null;
  baseY: number;
}): ViewportDrainAction {
  if (input.pinnedToBottom) return { kind: "bottom" };
  if (input.pendingBottomOffset === null) return { kind: "none" };
  return { kind: "line", line: Math.max(0, input.baseY - input.pendingBottomOffset) };
}
