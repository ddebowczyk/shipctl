...
export interface TerminalPaintRequest {
  ...
  /**
   * Browser cursor-blink preference. When absent, use the host cursor mode.
   *
   * The host still owns cursor position, visibility, and shape. Blink timing
   * is presentation state and can be overridden by the application setting.
   */
...
export function planTerminalPaint(
  ...
  return {
    ...
    cursor: planCursor(
      state,
      request.metrics,
      request.cursorLit ?? true,
      request.cursorBlink ?? state.cursor.blinking,
    ),
  };
}
...
function planCursor(
  ...
  return {
    ...
    column: head,
    columns,
    visible: state.cursor.visible && (lit || !blinking),
