...
import { decodeSemanticTerminalEvent } from "./terminalEventDecoder.ts";
...
export function createSemanticTerminalAttachmentBootstrap(
  onEvent: (event: SemanticTerminalWireEvent) => void,
  observeDecode?: (milliseconds: number) => void,
): TerminalAttachmentBootstrap {
  const buffered: SemanticTerminalWireEvent[] = [];
  let active = false;

  return {
    deliver(raw) {
      const startedAt = performance.now();
      const event = decodeSemanticTerminalEvent(raw);
