...
export const TERMINAL_CAPABILITY_REGISTER: readonly CapabilityEntry[] = [
  // ---- Screen, cursor and geometry -------------------------------------
  {
    ...
    proof: { by: "lane", test: "modules/semantic-terminal/frontend/tests/semanticTerminalViewSession.test.ts" },
    note: "The webview builds the semantic surface now — "
      + "terminalBrowserSession.ts's semantic binding — so the rows a "
      + "person sees are the host's projection through the model, and no local "
  ...
  },
  {
    ...
    note: "The shape and whether it blinks are Ghostty's own render state, "
      ...
      + "blinks at today — 600 ms — because product parity is the authority and "
      + "the host says only whether, never when.",
  },
  {
    id: "geometry.fit",
  ...
  // ---- Surface lifecycle -------------------------------------------------
  {
    ...
    proof: {
      by: "lane",
      test: "modules/semantic-terminal/frontend/tests/semanticTerminalViewSession.test.ts",
    },
  },
  {
  ...
  // ---- Input --------------------------------------------------------------
  {
    ...
    capability: "A browser keyboard, pointer, composition or focus event "
      ...
      + "on the client side",
    disposition: "implemented",
    proof: { by: "lane", test: "modules/semantic-terminal/frontend/tests/browserInteraction.test.ts" },
  ...
  },
  {
    ...
    proof: {
      by: "lane",
      test: "modules/semantic-terminal/frontend/tests/semanticTerminalAttachmentController.test.ts",
    },
  ...
  },
  {
    ...
    note: "Enumerated late: the product has it through xterm, and no entry "
      ...
      + "answer returned in its focused text area, selected. Command-C stays "
      + "the platform's copy gesture; the surface context menu writes the same "
      + "host selection. A person must confirm both gestures and the "
      + "wrapped-line result with the packaged interaction procedure.",
  },
  {
  ...
  // ---- Effects and integrations ------------------------------------------
  {
    ...
    proof: {
      by: "lane",
      test: "modules/semantic-terminal/frontend/tests/browserInteraction.test.ts",
    },
  },
  {
  ...
  },
  {
    ...
    disposition: "implemented",
    proof: { by: "lane", test: "core/frontend/terminal-host/tests/terminalSessions.test.ts" },
  },
  {
    ...
    note: "Each OSC 52 clipboard-write effect produces one visible refusal. "
      + "This child-requested effect does not call a clipboard API or expose "
  ...
  },
  {
    ...
    note: "A reader who scrolls back is shown host history rows above the live "
      ...
      + "displays (tests/semanticTerminalViewSession.test.ts).",
  },
  {
  ...
  },
  {
    ...
    disposition: "implemented",
    proof: { by: "lane", test: "core/frontend/terminal-host/tests/terminalRetention.test.ts" },
  },
