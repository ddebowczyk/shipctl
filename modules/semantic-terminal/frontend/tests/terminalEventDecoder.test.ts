...
const contract: Contract = JSON.parse(
  readFileSync(new URL("../fixtures/terminalEventContract.json", import.meta.url), "utf8"),
);
...
test("every declared variant decodes", () => {
  for (const variant of contract.variants) {
    const decoded = decodeSemanticTerminalEvent(sample(variant.tag));
...
test("a variant the host has not declared is rejected, not ignored", () => {
  assert.throws(
    () => decodeSemanticTerminalEvent({ event: "semantic_frame", sequence: 1 }),
    TerminalEventDecodeError,
  );
});
...
test("a required field missing from any variant is rejected", () => {
  for (const variant of contract.variants) {
    for (const field of Object.keys(variant.fields)) {
      ...
      assert.throws(
        () => decodeSemanticTerminalEvent(payload),
        TerminalEventDecodeError,
...
test("a sequence outside the exact integer range is rejected", () => {
  for (const bad of [0, -1, 1.5, MAX_EXACT_JSON_INTEGER + 2, Number.NaN, Number.POSITIVE_INFINITY]) {
    assert.throws(
      () => decodeSemanticTerminalEvent({ ...sample("detached"), sequence: bad }),
      TerminalEventDecodeError,
...
test("a nested replay field is validated, not trusted", () => {
  ...
  assert.throws(() => decodeSemanticTerminalEvent(payload), TerminalEventDecodeError);
});
...
test("the semantic state is validated, not trusted", () => {
  ...
  assert.throws(() => decodeSemanticTerminalEvent(wrongType), TerminalEventDecodeError);
});
...
test("occurrences keep the order the host reported", () => {
  ...
  const decoded = decodeSemanticTerminalEvent(payload);
...
test("the semantic path carries no child bytes", () => {
  const decoded = decodeSemanticTerminalEvent(sample("screen"));
...
test("a non-object payload is rejected", () => {
  for (const bad of [null, 7, "output", [{ event: "output" }]]) {
    assert.throws(() => decodeSemanticTerminalEvent(bad), TerminalEventDecodeError);
  }
});
