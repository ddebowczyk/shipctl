/**
 * A semantic module view session with no parser under it.
 *
 * The session is owned by semantic-terminal. It is given a client model rather
 * than an output queue, and a runtime that takes meaning rather than bytes.
 */
...
const fixture = JSON.parse(
  readFileSync(
    new URL("../fixtures/terminalScreenFixture.json", import.meta.url),
    "utf8",
  ),
...
const anchorFixture = JSON.parse(
  readFileSync(
    new URL("../fixtures/terminalAnchorFixture.json", import.meta.url),
    "utf8",
  ),
) as Record<string, unknown>;
...
const historyRow = (
  JSON.parse(
    readFileSync(
      new URL("../fixtures/terminalHistoryFixture.json", import.meta.url),
      "utf8",
    ),
  ) as { rows: unknown[] }
).rows[0];
...
function screenEvent(sequence: number, scrollbackRows?: number): SemanticTerminalWireEvent {
  ...
  return { ...event, sequence, state } as SemanticTerminalWireEvent;
}
...
class Harness {
  ...
  inputOutcome: TerminalInputOutcome = { status: "accepted", encodedBytes: 1 };
  ...
  emit: ((event: SemanticTerminalWireEvent) => void) | null = null;

  #semanticSink: ((input: TerminalInput) => void) | null = null;
  #frames: (() => void)[] = [];
  #deferred: Array<{ cancelled: boolean; task: () => void }> = [];
  ...
  constructor() {
    ...
    this.ports = {
      ...
      timing: {
        ...
        defer: (task) => {
          const deferred = { cancelled: false, task };
          this.#deferred.push(deferred);
          return () => { deferred.cancelled = true; };
        },
        fontsReady: () => null,
      },
      recordDiagnostic: (event) => this.diagnostics.push(event),
      reportEffect: (effect) => {
  ...
  async runDeferred(): Promise<void> {
    for (const deferred of this.#deferred.splice(0)) {
      if (!deferred.cancelled) deferred.task();
    }
    await settle();
  }
  ...
  async #attach(
    ...
    return {
      ...
      live: true,
      snapshot: {
        descriptor: { revision: baselineRevision() },
        sequenceBoundary: 0,
...
test("a concealed reveal cannot focus its hidden terminal", async () => {
  const h = new Harness();
  const session = startSemanticTerminalViewSession(h.ports);

  session.conceal();
  await h.frame();
  await h.runDeferred();
  assert.equal(h.trace.includes("attach"), false);
  assert.equal(h.trace.includes("focus"), false);

  session.reveal();
  await h.frame();
  await h.runDeferred();
  assert.equal(h.trace.filter((entry) => entry === "attach").length, 1);
  assert.equal(h.trace.filter((entry) => entry === "focus").length, 1);
});

test("the attachment's baseline becomes model state, with no bytes anywhere", async () => {
  const h = new Harness();
  startSemanticTerminalViewSession(h.ports);
  await h.frame();
...
test("input diagnostics identify control keys without retaining printable text", () => {
  const mods = {
    shift: false,
    alt: false,
    ctrl: true,
  ...
  const controlC = semanticInputDiagnosticFacts({
    kind: "key",
    action: "press",
    code: "KeyC",
    text: "c",
  ...
  const controlZ = semanticInputDiagnosticFacts({
    kind: "key",
    action: "press",
    code: "KeyZ",
    text: "z",
  ...
  const printable = semanticInputDiagnosticFacts({
    kind: "key",
    action: "press",
    code: "KeyS",
    text: "s",
  ...
  const committedText = semanticInputDiagnosticFacts({ kind: "text", text: "secret" });

  assert.equal(controlC.code, "KeyC");
  assert.equal(controlC.ctrl, true);
  assert.equal(controlZ.code, "KeyZ");
  assert.equal("code" in printable, false);
  assert.equal("text" in printable, false);
  assert.deepEqual(committedText, { kind: "text" });
  assert.equal(JSON.stringify([printable, committedText]).includes("secret"), false);
});
...
test("bytes arriving for a session with no parser are reported, not dropped", async () => {
  ...
  h.emit?.({ event: "output", sequence: 1, data: [104, 105] } as SemanticTerminalWireEvent);
  await settle();
