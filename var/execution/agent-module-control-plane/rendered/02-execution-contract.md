# Execution contract — Execution order and task contract
...
```text
/Users/ddebowczyk/projects/_ext_experiments/shep/docs/plans/20260808-090754-agent-module-control-plane/09-execution-order-and-task-contract.md
```
...
This is a governance chapter. Use it to align, sequence, and verify the plan;
do not duplicate product implementation assigned to later phase chapters.

Create action plans in:

```text
var/execution/agent-module-control-plane/action-plans/02-execution-contract
```

## Execution process

1. Claim this subepic (`shep-btu.2`) and record the live branch, commit,
...
11. Review the exact diff, stage only phase-owned changes, and commit them. The
    ...
    `phase Execution contract: complete <outcome>`.
...
- The phase commit exists and its subject identifies `Execution contract`.
- This subepic and all descendants are closed before execution advances.

## Transition

- Previous ordered subepic: Agent-operated capability module control plane.
- Next ordered subepic: Named instance specification.
