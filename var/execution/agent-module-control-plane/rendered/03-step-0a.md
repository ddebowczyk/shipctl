...
```text
/Users/ddebowczyk/projects/_ext_experiments/shep/docs/plans/20260808-090754-agent-module-control-plane/00a-named-instance-spec.md
```
...
```text
var/execution/agent-module-control-plane/action-plans/03-step-0a
```

## Execution process

1. Claim this subepic (`shep-btu.3`) and record the live branch, commit,
...
11. Review the exact diff, stage only phase-owned changes, and commit them. The
    ...
    `phase Step 0A: complete <outcome>`.
...
- The phase commit exists and its subject identifies `Step 0A`.
- This subepic and all descendants are closed before execution advances.

## Transition

- Previous ordered subepic: Execution order and task contract.
- Next ordered subepic: Saved instance state specification.
