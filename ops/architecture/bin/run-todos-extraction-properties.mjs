...
const evidenceDirectory = path.join(
  repositoryRoot,
  "target/architecture-evidence/todos-extraction",
);
...
const typescriptTests = [
  ...
  "ops/architecture/tests/todosNativeExtraction.test.mjs",
  "modules/todos/frontend/tests/todoDocuments.test.ts",
  "modules/todos/frontend/tests/store.test.ts",
];
const environment = {
  ...process.env,
...
const rust = await exec("cargo", [
  ...
  "shipctl-core",
  "--no-fail-fast",
  "architecture_provider_project_documents_",
  "--",
  "--nocapture",
], { cwd: repositoryRoot, env: environment });
...
const replayCommand = `node ops/architecture/bin/run-todos-extraction-properties.mjs --seed=${seed}`;
const properties = [
  {
    propertyId: "PROP-D-PARITY-001",
    testId: "architecture.provider.project-documents.parity.property",
    language: "rust",
    library: "proptest",
    version: "1.11.0",
    classifications: {
      capability: ["project-documents"],
      pilot: ["todos"],
      outcomes: ["success", "conflict", "durable-write"],
    },
  },
  {
    propertyId: "PROP-D-AUTHORITY-001",
    testId: "architecture.provider.project-documents.authority.property",
    language: "rust",
    library: "proptest",
    version: "1.11.0",
    classifications: {
      capability: ["project-documents"],
      authorization: ["allowed", "unknown", "disposed", "grant-denied", "scope-denied"],
    },
  },
  {
    propertyId: "PROP-D-OWNERSHIP-001",
    testId: "architecture.provider.project-documents.ownership.property",
    language: "rust",
    library: "proptest",
    version: "1.11.0",
    classifications: {
      capability: ["project-documents"],
      ownership: ["activation-access", "project-document"],
    },
  },
  {
    propertyId: "PROP-D-CLOSURE-001",
    testId: "architecture.todos-native-extraction-closure.property",
    language: "typescript",
    library: "fast-check",
