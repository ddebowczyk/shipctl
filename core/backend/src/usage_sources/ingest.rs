...
/// Ingest only the reviewed source identities admitted for this request.
pub fn ingest_sources(conn: &Connection, source_ids: &HashSet<String>) -> bool {
    let mut all_done = true;

    if source_ids.contains("claude") {
    ...
    if source_ids.contains("antigravity") {
