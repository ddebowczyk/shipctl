...
use super::contracts::{
    MessageContractError, MessageDeclarations, MessageEnvelope, MessageRouteSnapshot,
    MessageTypeId, ModuleMessageAuthority, MESSAGE_CONTRACT_SCHEMA_VERSION,
};
use super::diagnostics::{
    BRIDGE_CLOSED, HANDLER_FAILED, HANDLER_UNAVAILABLE, INVALID_IDENTIFIER,
    NO_ACTIVE_CHANNEL_OWNER, UNAUTHORIZED_SENDER,
};
use super::routes::{PreparedRegistration, RegistrationHandlers};
use super::runtime::{ActivationRuntimeObservation, EndpointRuntimeObservation, RuntimeMessageBus};
...
impl FrontendBridge {
    ...
    fn authority_for_module_publish(
        ...
        if matching.len() > 1 {
            return Err(MessageContractError::new(
                super::diagnostics::DUPLICATE_CHANNEL_OWNER,
...
impl MessageBusBridgeService {
                ...
                tasks.push(tokio::spawn(async move {
                    loop {
                        match subscription.recv_delivery().await {
                            ...
                            Err(error) if error.code == super::diagnostics::SUBSCRIBER_LAG => {
                                continue;
                            }
                            Err(_) => break,
                        }
    ...
    pub async fn send(
        ...
        activation_id: &str,
        envelope: MessageEnvelope,
    ) -> Result<super::contracts::DeliveryReceipt, MessageContractError> {
        let bridge = self.bridge(bridge_id).await?;
        let authority = {
            let _transaction = bridge.transaction.lock().await;
    ...
    pub async fn publish(
        ...
        activation_id: &str,
        envelope: MessageEnvelope,
    ) -> Result<super::contracts::PublishReceipt, MessageContractError> {
        let bridge = self.bridge(bridge_id).await?;
        let authority = {
            let _transaction = bridge.transaction.lock().await;
            bridge.authority(activation_id)?
...
mod tests {
    ...
    use crate::message_bus::{
        ...
        MessageSchemaDescriptor, MessageTypeContract, RouteEndpointRef,
    };
