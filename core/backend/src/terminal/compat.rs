...
//!
//! Retention is deliberately absent: [`super::retention`] is the single
...
fn grid_point<'t>(
    ...
    x: u16,
    y: u16,
) -> libghostty_vt::screen::GridRef<'t> {
    terminal
...
/// *amount* of history is measured in [`super::retention`]; this proves the
...
fn the_child_owns_the_palette_and_the_default_colors() {
    ...
    assert_eq!(
        palette.get(libghostty_vt::style::PaletteIndex(1)),
        RgbColor {
            r: 0x01,
            g: 0x02,
            b: 0x03
...
fn facts_copied_out_stay_valid_after_the_terminal_moves_on() {
    ...
    assert_eq!(
        ...
        terminal
            .color_palette()
            .expect("palette")
            .get(libghostty_vt::style::PaletteIndex(1))
    );
