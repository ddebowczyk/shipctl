//! A recorded provider corpus of child output and the state it produces.
//!
...
//!
//! ```sh
//! cargo test -p shipctl-core --lib semantic_terminal::traces::tests::record -- --ignored
//! ```
//!
...
fn corpus_directory() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("fixtures/semantic-terminal-traces")
}
...
mod tests {
    ...
    fn recorded_bytes(trace: &Trace) -> Vec<u8> {
        fs::read(bytes_path(trace)).unwrap_or_else(|error| {
            panic!(
                "trace {} is missing its recording ({error}); record it with `cargo test -p shipctl-core --lib semantic_terminal::traces::tests::record -- --ignored`",
                trace.name
            )
        })
    }
