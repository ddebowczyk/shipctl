//! Semantic terminal input, owned and encoded by the native provider.
//!
...
/// Build the Ghostty focus event for the temporary core replay bridge.
pub fn focus_event(gained: bool) -> focus::Event {
    if gained {
        focus::Event::Gained
    } else {
        focus::Event::Lost
...
impl TerminalKeyEvent {
    /// Build the Ghostty key event for the temporary core replay bridge.
    pub fn build(&self) -> Result<key::Event<'static>, String> {
...
impl TerminalMouseEvent {
    /// Build the Ghostty mouse event for the temporary core replay bridge.
    pub fn build(&self) -> Result<(mouse::Event<'static>, mouse::EncoderSize), String> {
