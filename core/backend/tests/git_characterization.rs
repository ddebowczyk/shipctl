...
impl git::GitProjectCatalog for FixedProjectRoot {
    fn registered_project_paths(&self) -> Result<Vec<String>, String> {
        Ok(vec![self.root.to_string_lossy().to_string()])
    }
}
...
fn project_authority_accepts_only_the_registered_root() {
    ...
    let service = git::GitService::new(Arc::new(FixedProjectRoot {
        root: registered.clone(),
    }));
    let actor = git::GitActor {
        module_id: "shipctl.git".to_string(),
        activation_id: "git-characterization".to_string(),
    };

    assert!(!service
        .is_repository(&actor, registered.to_str().unwrap())
        .unwrap());
    assert_eq!(
        service
            .is_repository(&actor, fixture.path().to_str().unwrap())
            .unwrap_err()
            .code,
        git::GIT_INVALID_PROJECT
    );
    assert_eq!(
        service
            .is_repository(&actor, registered.join("child").to_string_lossy().as_ref(),)
            .unwrap_err()
            .code,
        git::GIT_INVALID_PROJECT
    );
