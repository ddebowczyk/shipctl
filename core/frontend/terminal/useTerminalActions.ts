...
export function useTerminalActions(
  ...
  const observeTerminalSession = useCallback(async (
    ...
    const attachment = await TERMINAL_CLIENT_RUNTIME.attach(
      ...
      (event) => {
        if (event.event === "output") {
          ...
          deliver({ type: "replay", data: event.replay.bytes });
    ...
    if (!attachment.live) {
      deliver({
        type: "exit",
        exitCode: attachment.snapshot.descriptor.exit?.code ?? null,
      });
    }
    attachment.activate();
  ...
  const spawnBlankShell = useCallback(async (columns: number, rows: number) => {
