...
/**
 * Rows the transitional xterm buffer keeps.
 *
 * This is a renderer safety limit, not the product's retention policy. It is
 * deliberately not derived from the byte budget: converting bytes to rows would
 * require a bytes-per-row constant that the host does not have. It disappears
 * with xterm.
 */
export const TRANSITIONAL_RENDERER_SCROLLBACK_ROWS = 10_000;
