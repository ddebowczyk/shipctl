...
import type { TerminalClientModel, TerminalDamageModel } from "./terminalClientModel.ts";
 ...
 *
 * The rate the product blinks at today: `@xterm/addon-webgl`'s
 * `CursorBlinkStateManager` restarts its interval at 600 ms, and that addon is
 * the renderer this application selects. Parity is the authority — nothing here
 * chose a rate, and the host says only whether a cursor blinks, never when.
 */
...
export class TerminalCellPresenter {
  ...
  start(): void {
    ...
    this.#unsubscribe = this.#ports.model.subscribe((_state, damage) => {
  ...
  #paint(): void {
    ...
    this.#full = false;
    this.#rows.clear();
    this.#blink(displayed.cursor.visible && displayed.cursor.blinking);
