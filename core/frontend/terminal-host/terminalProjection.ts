...
): LocatedTerminal | undefined {
  for (const [projectPath, project] of Object.entries(state)) {
    const index = project.tabs.findIndex(
      (entry) => entry.kind === "terminal" && entry.terminalId === terminalId,
    );
    if (index >= 0) {
      return { projectPath, index, tab: project.tabs[index] as TerminalTabData };
    }
  }
  return undefined;
}
...
export function upsertTerminalProjection(
  ...
  if (existing && existing.tab.terminalRevision > descriptor.revision) return state;
  ...
  if (existing?.projectPath === projectPath) {
    ...
    tabs[existing.index] = tab;
    return {
      ...state,
      [projectPath]: { ...project, tabs },
    };
  }
