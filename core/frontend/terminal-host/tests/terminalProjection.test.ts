...
test("a descriptor refresh preserves the selected tab and tab order", () => {
  const projectPath = "/repos/shared";
  const initial = reconcileTerminalProjection(EMPTY, [
    descriptor("one", 1, "running", projectPath),
    descriptor("two", 1, "running", projectPath),
  ]);
  const selected = {
    ...initial,
    [projectPath]: { ...initial[projectPath], activeTabId: "terminal:two" },
  };

  const refreshed = upsertTerminalProjection(
    selected,
    descriptor("two", 2, "running", projectPath),
  );

  assert.equal(refreshed[projectPath].activeTabId, "terminal:two");
  assert.deepEqual(
    refreshed[projectPath].tabs.map((tab) => tab.id),
